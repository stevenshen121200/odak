...
class multi_color_hologram_optimizer():
    ...
    def __init__(self,
        ...
        self.number_of_slms = number_of_slms
    ...
    def direct_phase_constrain(self, phase, phase_offset):
        ...
        phase_only = (phase_only+np.pi) % (2 * np.pi)
        #phase_only = torch.clip(phase_only+np.pi, 0, 2 * np.pi)
        #phase_only = torch.sigmoid(phase_only)*2*np.pi
    ...
    def gradient_descent(self, number_of_iterations=100, weights=[1., 1., 0., 0.]):
        ...
        for step in t:
            ...
            for depth_id in t_depth:
                ...
                del loss
            description = "Loss:{:.3f} Loss Image:{:.3f} Peak Amp:{:.1f} Learning rate:{:.4f}".format(total_loss,
                                                                                                      loss_image.item(),
                                                                                                      self.peak_amplitude,
                                                                                                      learning_rate)
    ...
    def gradient_descent_multi_layers(self, number_of_iterations=100, weights=[1., 1., 0., 0.]):
        """
        Function to optimize multiplane phase-only holograms using stochastic gradient descent.
        ...
        for step in t:
            ...
            for depth_id in t_depth:
                ...
                for frame_id in range(self.number_of_frames):
                    ...
                    if self.double_phase:
                        phase, loss_phase = self.double_phase_constrain(
                            self.phase[frame_id][slm_id],
                            self.offset[frame_id][slm_id]
                        )
                    else:
                        phase, loss_phase = self.direct_phase_constrain(
                            self.phase[frame_id][slm_id],
                    ...
                    for channel_id in range(self.number_of_channels):
                        ...
                        middle_field = self.propagator(hologram, channel_id, 0)
                        middle_fields.append(middle_field)
                        if self.number_of_slms == 1:
                            reconstruction_field = middle_field
                            intensity = calculate_amplitude(middle_field) ** 2
                            reconstruction_intensities[frame_id, channel_id] += intensity
                    hologram_phases[frame_id][slm_id] = phase.detach().clone()
                    ...
                    if self.number_of_slms > 1:
                        ...
                        for channel_id in range(self.number_of_channels):
                            ...
                            middle_field = middle_fields[channel_id] * hologram
                            reconstruction_field = self.propagator(middle_field, channel_id, depth_id)
                            intensity = calculate_amplitude(reconstruction_field) ** 2
                            reconstruction_intensities[frame_id, channel_id] += intensity
                        hologram_phases[frame_id][slm_id] = phase.detach().clone()
                loss_laser = self.l2_loss(
    ...
    def optimize(self, number_of_iterations=100, weights=[1., 1., 1.], bits=8):
        ...
        """
        self.init_optimizer()
        hologram_phases = self.gradient_descent_multi_layers(
            number_of_iterations=number_of_iterations,
            weights=weights
        )
        hologram_phases = quantize(hologram_phases % (2 * np.pi), bits=bits,
                                   limits=[0., 2 * np.pi]) / 2 ** bits * 2 * np.pi
        torch.no_grad()
